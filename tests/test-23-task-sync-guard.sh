...
exec_in_manager bash -lc "mkdir -p /root/hiclaw-fs/shared/tasks/${TASK_ID}/base && printf '%s' '{\"task_id\":\"${TASK_ID}\",\"status\":\"assigned\"}' > /root/hiclaw-fs/shared/tasks/${TASK_ID}/meta.json && printf '%s' 'spec-v1' > /root/hiclaw-fs/shared/tasks/${TASK_ID}/spec.md && . /opt/hiclaw/scripts/lib/hiclaw-env.sh && mc cp /root/hiclaw-fs/shared/tasks/${TASK_ID}/meta.json \"\${HICLAW_STORAGE_PREFIX}/shared/tasks/${TASK_ID}/meta.json\" && mc cp /root/hiclaw-fs/shared/tasks/${TASK_ID}/spec.md \"\${HICLAW_STORAGE_PREFIX}/shared/tasks/${TASK_ID}/spec.md\""
...
REMOTE_META=$(exec_in_manager bash -lc ". /opt/hiclaw/scripts/lib/hiclaw-env.sh; mc cat \"\${HICLAW_STORAGE_PREFIX}/shared/tasks/${TASK_ID}/meta.json\"")
REMOTE_PLAN=$(exec_in_manager bash -lc ". /opt/hiclaw/scripts/lib/hiclaw-env.sh; mc cat \"\${HICLAW_STORAGE_PREFIX}/shared/tasks/${TASK_ID}/plan.md\"")
